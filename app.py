...
def get_movement(msv):
    ...
    if m <= 10000:
        return 0.75
    return 1.0  # one position/month for MSV > 10k
...
with st.sidebar:
    ...
    st.session_state.ctr_df = st.data_editor(
        ...
        column_config={
            "CTR": st.column_config.NumberColumn(
                "CTR (%)", min_value=0.0, max_value=100.0
            )
        },
    ...
    st.session_state.seasonality_df = st.data_editor(
        ...
        column_config={
            "Adjustment (%)": st.column_config.NumberColumn(
                "Adjustment (%)", min_value=-100.0, max_value=100.0
            )
        },
        use_container_width=True,
        hide_index=True,
    ...
    fs_ctr = st.number_input(
        ...
        min_value=0.0, max_value=100.0, value=18.0
    )
    ...
    if not st.session_state.launch_month_df.empty:
        proj = st.selectbox(
            ...
            st.session_state.launch_month_df["Project"].tolist(),
            key="paid_project_selector"
        )
        st.session_state.paid_listings[proj] = st.slider(
            f"{proj} Paid Listings", 0, 10,
            st.session_state.paid_listings.get(proj, 2),
            key=f"paid_{proj}"
        )
...
with tabs[0]:
    ...
    df["Current Position"] = pd.to_numeric(
        df.get("Current Position",0),
        errors="coerce"
    ).fillna(0)
    ...
    launch_map = st.session_state.launch_month_df.set_index("Project")[
        "Launch Date"
    ].to_dict()
    ...
    for scenario in ["High","Medium","Low"]:
        for _, r in filtered.iterrows():
            proj, msv, pos = (
                r["Project"], r["MSV"], r["Current Position"]
            )
            ...
            for i in range(1, 25):
                ...
                if date >= launch:
                    # month-1 snap for any >20 → 20
                    if i==1 and cur_pos>20:
                        cur_pos = 20
                    elif i>1:
                    ...
                    if pi <= st.session_state.ctr_df["Position"].max():
                        ...
                        if scenario=="High":
                            ...
                            ctr = base_ctr * (
                                1 - 0.05*st.session_state.paid_listings.get(proj,0)
                            )
                        ...
                        adj = st.session_state.seasonality_df.loc[
                            st.session_state.seasonality_df["Month"]==
                            date.strftime("%B"),
                            "Adjustment (%)"
                        ].iloc[0]
    ...
    plot_df = (
        rec_df.groupby(["Scenario","Date"])["Adjusted Clicks"]
    ...
    if end_date < start_date:
        ...
        fig = px.line(plot_df[mask], x="Date", y="Clicks",
        ...
        pivot = (
            summ.pivot_table(
                index=["Month","SortKey"],
                columns="Scenario", values="Clicks", aggfunc="sum"
            )
        ...
        med = (
            rec_df[rec_mask].groupby("Project")["Raw Clicks"]
            .sum().reset_index()
        )
        kc = filtered.groupby("Project")["Keyword"]\
                     .count().reset_index(name="Keyword Count")
        combo = kc.merge(med, on="Project", how="left").fillna(0)

        fig2 = go.Figure()
        if not combo.empty:
            fig2.add_bar(
                x=combo["Project"], y=combo["Raw Clicks"],
                name="Medium Clicks"
            )
            fig2.add_scatter(
                x=combo["Project"], y=combo["Keyword Count"],
                mode="lines+markers", name="Keyword Count",
                yaxis="y2"
            )
            fig2.update_layout(
                ...
                yaxis2=dict(
                    overlaying="y", side="right", title="Keyword Count"
                ),
        ...
        for proj in filtered["Project"].unique():
            # existing URLs
            for url in filtered[filtered["Project"]==proj]["Current URL"].dropna().unique():
                clicks3 = rec_df[
                    (rec_df["Scenario"]=="Medium") &
                    (rec_df["Project"]==proj) &
                    (rec_df["URL"]==url) &
                ...
                weighted = (
                    (wr["Current Position"]*wr["MSV"]).sum()/
                    wr["MSV"].sum()
            ...
            blanks = filtered[
                (filtered["Project"]==proj)&
                (filtered["Current URL"].isna()|
                 (filtered["Current URL"]==""))
            ]
            for _, br in blanks.iterrows():
                kw = br["Keyword"]
                clicks3 = rec_df[
                    (rec_df["Scenario"]=="Medium") &
                    (rec_df["Project"]==proj) &
        ...
        st.dataframe(
            actions_df[[
                "Project","Action","URL","Weighted Avg Rank",
                "3-Month Clicks","6-Month Clicks"
            ]],
